...
class FeeMarket {
    mempool = new Map();
    feeHistory = [];
    maxMempoolSize = 30000; // Default 30k transactions
    minRelayFeeRate = BigInt(1); // 1 satoshi/byte minimum
    blockHeight = 0;
    targetBlocks = [1, 2, 3, 6, 12, 24, 48, 144]; // Bitcoin-style targets
    /**
     * Estimate fee for a transaction to confirm in target blocks
     */
    estimateFee(targetBlocks, txSize) {
        ...
        const sortedRates = [...historicalRates].sort((a, b) => a < b ? -1 : a > b ? 1 : 0);
     ...
     */
    getHistoricalFeeRates(targetBlocks) {
        const rates = [];
     ...
     */
    getCurrentMempoolFeeRate(targetBlocks) {
     ...
     */
    calculateVariance(rates, median) {
        if (rates.length === 0)
            return 0;
        let sum = 0n;
     ...
     */
    createBumpedTransaction(oldTx, bumpAmount) {
     ...
     */
    findDependentTransactions(txid) {
        const dependencies = [];
     ...
     */
    updateFeeHistory(blockTransactions) {
        const feeRates = [];
