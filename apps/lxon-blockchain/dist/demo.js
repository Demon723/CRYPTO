...
function generateIndependentTransactions(count) {
    const txs = [];
...
function generateConflictingTransactions(count) {
    const txs = [];
...
async function runBlockStmBenchmark() {
    ...
    for (const threads of [1, 2, 4, 8]) {
        const engine = new block_stm_1.BlockSTMEngine(independentTxs);
        ...
        console.log(`Block-STM [${threads} Thread(s)]: ${duration.toFixed(3)} ms ` +
            `| Speedup: ${speedup.toFixed(2)}x`);
    }
    console.log(`\n--- Workload B: High Contention (DEX Hotspot) ---`);
    console.log(`Sequential Execution: ${seqTimeConflict.toFixed(3)} ms`);
    for (const threads of [1, 2, 4, 8]) {
        const engine = new block_stm_1.BlockSTMEngine(conflictingTxs);
...
async function runStorageBenchmark() {
    ...
    console.log('================================================================');
    const engine = new storage_1.MonadDBStorageEngine('/dev/nvme0n1');
    const lookupRequests = Array.from({ length: 40 }, (_, i) => [
...
async function runZkVMProofBenchmark() {
    ...
    const prover = new zkvm_1.RISCVzkVMProverStack(binaryELF);
