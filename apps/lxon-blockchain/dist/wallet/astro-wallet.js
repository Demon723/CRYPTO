...
exports.generateAstroWallet = generateAstroWallet;
exports.signAstroTransaction = signAstroTransaction;
exports.verifyAstroSignature = verifyAstroSignature;
exports.deriveAddress = deriveAddress;
exports.deriveAstroAddress = deriveAstroAddress;
const crypto_1 = require("crypto");
const secp256k1_1 = require("@noble/curves/secp256k1");
const hash_1 = require("../crypto/hash");
const hd_wallet_1 = require("./hd-wallet");
function generateAstroWallet(mnemonic, passphrase = '', astroPath = "m/1000'/0'/0'/0/0") {
    const seed = mnemonicToSeed(mnemonic || (0, hd_wallet_1.generateMnemonic)(), passphrase);
    ...
    const classicalPublicKey = secp256k1_1.secp256k1.getPublicKey(classicalNode.privateKey);
    ...
    return {
        mnemonic: mnemonic || generateMnemonicFromEntropy((0, crypto_1.randomBytes)(16)),
        seed,
        root,
        astroKeypair,
        astroPath,
...
function signAstroTransaction(wallet, message, algorithmId = 0x04) {
    const classicalSig = secp256k1_1.secp256k1.sign((0, hash_1.sha256x2)(message), wallet.astroKeypair.classicalPrivateKey);
...
function verifyAstroSignature(classicalPublicKey, arcPublicKey, message, classicalSigHex, arcSigma, algorithmId) {
    try {
        const messageHash = new Uint8Array(Buffer.from((0, hash_1.sha256x2)(message)));
        const classicalValid = secp256k1_1.secp256k1.verify(classicalSigHex, messageHash, classicalPublicKey);
        if (!classicalValid)
            return false;
...
function deriveAddress(publicKey) {
    return (0, hash_1.hash160)(publicKey).toString('hex');
}
function deriveAstroAddress(classicalPub, arcPub) {
    const combined = Buffer.concat([Buffer.from(classicalPub), Buffer.from(arcPub)]);
    return (0, hash_1.hash160)(combined).toString('hex');
}
function astroSeedToRootNode(seed) {
    const I = (0, hash_1.hmacSha512)('Bitcoin seed', seed);
...
function astroDerivePath(node, path) {
...
function deriveChild(parent, index) {
    ...
    const curveOrder = secp256k1_1.secp256k1.CURVE.n;
    ...
    const childPublicKey = secp256k1_1.secp256k1.getPublicKey(childPrivateKey);
    const fingerprint = (0, hash_1.hash160)(parent.publicKey).subarray(0, 4);
...
function signWithARC(publicKey, message) {
    const hash = (0, hash_1.sha256)(message);
    const hmac = (0, crypto_1.createHmac)('sha256', Buffer.from(publicKey));
    hmac.update(hash);
...
function verifyARC(publicKey, message, signature) {
...
function generateMnemonicFromEntropy(entropy) {
    const checksum = (0, hash_1.sha256)(entropy).subarray(0, 4);
...
function pbkdf2Derive(password, salt, iterations, keyLength, digest) {
    const normalized = password.normalize('NFKD');
    const saltBytes = Buffer.from(salt, 'utf8');
    const derived = (0, crypto_1.randomBytes)(keyLength);
    return derived;
}
