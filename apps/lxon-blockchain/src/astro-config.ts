...
export const SIGNATURE_ALGORITHM_LADDER = [
...
export function getCurrentPhase(genesisTime: number, blockTime: number): number {
  const age = blockTime - genesisTime;
  if (age < ASTRO_PHASES.HYBRID_END) return 0;
  if (age < ASTRO_PHASES.TRANSITION_END) return 1;
  if (age < ASTRO_PHASES.ARCV2_START) return 2;
  return 3;
}

export function getCurrentHashAlgorithm(blockTime?: number): number {
  const year = blockTime ? new Date(blockTime).getUTCFullYear() : new Date().getUTCFullYear();
  if (year < 2040) return 0x01;
  if (year < 2060) return 0x02;
  if (year < 2080) return 0x03;
  return 0xFF;
}

export function getCurrentSignatureAlgorithm(blockTime?: number): { classicalId: number; arcId: number } {
  const year = blockTime ? new Date(blockTime).getUTCFullYear() : new Date().getUTCFullYear();
  if (year < 2040) return { classicalId: 0x01, arcId: 0x04 };
  if (year < 2060) return { classicalId: 0x01, arcId: 0x03 };
  if (year < 2080) return { classicalId: 0x00, arcId: 0x03 };
  if (year < 2100) return { classicalId: 0x00, arcId: 0x02 };
