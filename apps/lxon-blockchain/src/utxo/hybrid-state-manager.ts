...
export class HybridStateManager {
  ...
  private utxoIndex: Map<string, Set<string>> = new Map(); // address -> UTXO keys
  private blockHeight: number = 0;

  /**
   * Generate UTXO key for map lookup
   */
  private utxoKeyToString(key: UTXOKey): string {
   ...
   */
  private verifyScript(lockingScript: Buffer, unlockingScript: Buffer, witness?: Buffer[]): boolean {
  ...
  private verifyP2PKH(lockingScript: Buffer, unlockingScript: Buffer): boolean {
  ...
  private verifyP2WPKH(lockingScript: Buffer, witness: Buffer[]): boolean {
  ...
  private verifyP2TR(lockingScript: Buffer, witness: Buffer[]): boolean {
   ...
   */
  processTransaction(tx: HybridTransaction): { success: boolean; error?: string } {
    // Process UTXO inputs
   ...
   */
  private computeTransactionID(tx: HybridTransaction): string {
   ...
   */
  private bigIntToLE(value: bigint): Buffer {
   ...
   */
  async validateUTXOSpendsParallel(inputs: TransactionInput[]): Promise<boolean[]> {
  ...
  createCoinbaseTransaction(
    ...
    const coinbaseTx: HybridTransaction = {
      version: 2,
      inputs: [{
   ...
   */
  private createP2PKHScript(address: string): Buffer {
   ...
   */
  private decodeAddressToHash(address: string): Buffer {
