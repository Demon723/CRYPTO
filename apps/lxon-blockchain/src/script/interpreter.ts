...
export class ScriptInterpreter {
  ...
  private altStack: Buffer[] = [];
  private pc: number = 0;
  private script: Buffer;
  private context: ScriptContext;
  private conditionalDepth: number = 0;
  private opcount: number = 0;
  private maxOps: number = 201;
  private maxScriptLength: number = 10000;
  private maxStackSize: number = 1000;
  private maxAltStackSize: number = 1000;
  ...
  execute(): ScriptResult {
    ...
    while (this.pc < this.script.length) {
      ...
      if (opcode >= OpCode.OP_DATA_1 && opcode <= OpCode.OP_DATA_75) {
        ...
        this.pushData4();
      } else if (opcode === OpCode.OP_0) {
        this.stack.push(Buffer.from([]));
      } else if (opcode >= OpCode.OP_1 && opcode <= OpCode.OP_16) {
        this.stack.push(Buffer.from([opcode - OpCode.OP_1 + 1]));
  ...
  private pushData1(): void {
    const length = this.script[this.pc++];
    this.pushData(length);
  }

  private pushData2(): void {
    const length = this.script.readUInt16LE(this.pc);
    this.pc += 2;
    this.pushData(length);
  }

  private pushData4(): void {
    const length = Number(this.script.readUInt32LE(this.pc));
    this.pc += 4;
    this.pushData(length);
  }

  private executeOp(opcode: number): ScriptResult {
    switch (opcode) {
      case OpCode.OP_DUP:
        if (this.stack.length < 1) return this.fail('Stack underflow');
      ...
      case OpCode.OP_SWAP:
        if (this.stack.length < 2) return this.fail('Stack underflow');
      ...
      case OpCode.OP_ROT:
        if (this.stack.length < 3) return this.fail('Stack underflow');
      ...
      case OpCode.OP_2OVER:
        if (this.stack.length < 4) return this.fail('Stack underflow');
        this.stack.push(Buffer.from(this.stack[this.stack.length - 4]));
        this.stack.push(Buffer.from(this.stack[this.stack.length - 4]));
        return { success: true, stack: this.stack, altStack: this.altStack };

      case OpCode.OP_2ROT:
        if (this.stack.length < 6) return this.fail('Stack underflow');
      ...
      case OpCode.OP_NOTIF: {
        ...
        const required = opcode === OpCode.OP_NOTIF ? !cond : cond;
      ...
      case OpCode.OP_VERIFY: {
        if (this.stack.length < 1) return this.fail('Stack underflow');
        if (!this.isTruthy(this.stack.pop()!)) return this.fail('Verify failed');
        return { success: true, stack: this.stack, altStack: this.altStack };
      }
      ...
      case OpCode.OP_FROMALTSTACK:
        if (this.altStack.length < 1) return this.fail('Alt stack underflow');
      ...
      case OpCode.OP_ROLL: {
        ...
        if (n < 0 || n >= this.stack.length) return this.fail('Invalid pick/roll index');
      ...
      case OpCode.OP_EQUALVERIFY: {
        ...
        const b = this.stack.pop()!;
        const a = this.stack.pop()!;
        if (!a.equals(b)) return this.fail('EqualVerify failed');
        return { success: true, stack: this.stack, altStack: this.altStack };
      }
      ...
      case OpCode.OP_HASH160: {
        ...
        this.stack.push(Buffer.from(hash160(data)));
        return { success: true, stack: this.getStack(), altStack: this.getAltStack() };
      }
      ...
      case OpCode.OP_HASH256: {
        if (this.stack.length < 1) return this.fail('Stack underflow');
        const data = this.stack.pop()!;
        this.stack.push(Buffer.from(sha256(sha256(data))));
        return { success: true, stack: this.getStack(), altStack: this.getAltStack() };
      }
      ...
      case OpCode.OP_CHECKSIGVERIFY: {
        ...
        const pubkey = this.stack.pop()!;
        const sig = this.stack.pop()!;
        if (!this.verifySignature(sig, pubkey)) return this.fail('Checksig verify failed');
        return { success: true, stack: this.stack, altStack: this.altStack };
      }

      case OpCode.OP_CHECKMULTISIG: {
        const n = this.stack.pop();
        if (!n) return this.fail('Stack underflow');
        const nKeys = this.decodeNumber(n);
        if (this.stack.length < nKeys + 1) return this.fail('Stack underflow');
        const pubkeys: Buffer[] = [];
        for (let i = 0; i < nKeys; i++) {
          pubkeys.push(this.stack.pop()!);
        }
        const m = this.stack.pop();
        if (!m) return this.fail('Stack underflow');
        const mSigs = this.decodeNumber(m);
        const sigs: Buffer[] = [];
        for (let i = 0; i < mSigs; i++) {
          const sig = this.stack.pop();
          if (!sig) return this.fail('Stack underflow');
          sigs.push(sig);
        }
        let validCount = 0;
      ...
      case OpCode.OP_CHECKMULTISIGVERIFY: {
        ...
        if (!nm) return this.fail('Stack underflow');
        ...
        if (!mm) return this.fail('Stack underflow');
        ...
        if (validCount < mSigs) return this.fail('Checkmultisig verify failed');
        return { success: true, stack: this.stack, altStack: this.altStack };
      }
      ...
      case OpCode.OP_CHECKLOCKTIMEVERIFY: {
        ...
        if (this.context.lockTime < locktime) return this.fail('Locktime not satisfied');
        return { success: true, stack: this.stack, altStack: this.altStack };
      }
      ...
      case OpCode.OP_CHECKSEQUENCEVERIFY: {
        ...
        if (this.context.inputSequence < sequence) return this.fail('Sequence not satisfied');
        return { success: true, stack: this.stack, altStack: this.altStack };
      }
  ...
  private skipUnless(condition: boolean): ScriptResult {
    let depth = 0;
    while (this.pc < this.script.length) {
      const opcode = this.script[this.pc++];
  ...
  private skipOp(opcode: number): void {
    if (opcode >= OpCode.OP_DATA_1 && opcode <= OpCode.OP_DATA_75) {
      this.pc += opcode;
    } else if (opcode === OpCode.OP_PUSHDATA1) {
  ...
  private isTruthy(data: Buffer): boolean {
  ...
  private decodeNumber(data: Buffer): number {
  ...
  private encodeNumber(value: number): Buffer {
  ...
  private verifySignature(sig: Uint8Array, pubkey: Uint8Array): boolean {
    try {
      return secp256k1.verify(sig.subarray(0, 64), this.context.txDigest, pubkey);
    } catch {
      return false;
    }
  }

  private fail(reason: string): ScriptResult {
