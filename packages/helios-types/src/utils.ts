...
export function tierLabel(tier: number): string {
  const labels = ['Genesis', 'Solar', 'Main Sequence', 'Red Giant', 'Supernova'] as const;
  return labels[tier] ?? 'Unknown';
}

export function generateCardNumber(tokenId: number, tier: number): string {
...
export function parseTapToPayMessage(
  tokenId: number,
  to: string,
  value: bigint,
  data: string,
